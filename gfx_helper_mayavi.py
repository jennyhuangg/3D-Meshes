...
on the figure, since it will block until the figure is closed.
  ...
  fig.scene.camera.position = [7.7, 10.5, 17.8]
  fig.scene.camera.view_up = [0, 1, 0]
  #fig.scene.camera.position = [-13.5, 9.7, -14.5]
  #fig.scene.camera.view_up = [0.3, 0.9, 0.3]
...
Mayavi figure fig, here are some example calls:
  ...
  if len(verts.shape) != 2 or verts.shape[0] != 3:
    raise ValueError("'verts' must be 3xN")
